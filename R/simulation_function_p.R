...
#' 
#' @export
simulation_function_p = function(n_sites = 50, # number of sites
  ...
  out <- tryCatch(
    {
      ...
      # calculate several types of residuals & save everything
      {
        ...
        res2 <- cbind(res, 
                      ...
                      'alpha'        = alpha, # this is just the input alpha value. If it's NA, then alpha is calculated internally
                      'beta'         = beta) # just the input beta value. If it's NA, then beta is calculated internally
